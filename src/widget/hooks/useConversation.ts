...
export function useConversation(sessionId?: string): UseConversationReturn {
  ...
  const stateHistory = useRef<ConversationState[]>([ConversationState.GREETING]);
  ...
  const addInsight = useCallback((insight: CustomerInsight) => {
    setInsights(prev => {
      ...
      const exists = prev.some(i => 
        i.type === insight.type && i.value === insight.value
      );
      ...
      return prev.map(i => 
        i.type === insight.type && i.value === insight.value && i.confidence < insight.confidence
  ...
  const updateShoppingIntent = useCallback((intent: Partial<ShoppingIntent>) => {
    setContext(prev => {
      if (!prev) {return null;}
  ...
  const getConversationSummary = useCallback((): string => {
    if (!context) {return 'No conversation started';}
    ...
    const shoppingTopics = context.topicStack.filter(t => t.type === 'shopping').length;
