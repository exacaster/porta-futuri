...
export class CSVProcessor {
        ...
        chunk: (results) => {
          ...
          results.data.forEach((row: any) => {
            ...
            try {
              ...
              if (row.attributes) {
                row.attributes = this.parseJSONField(
                  row.attributes,
                  "attributes",
                );
                if (
                  row.attributes &&
                  !this.validateAttributes(row.attributes)
                ) {
                  errors.push({
                    type: "InvalidValue",
                    code: "INVALID_ATTRIBUTES",
                    message:
                      "Invalid attributes format (must be a JSON object)",
                    row: rowCount,
                    field: "attributes",
                  });
              ...
              if (row.comments) {
                ...
                if (row.comments && !this.validateComments(row.comments)) {
                  errors.push({
                    ...
                    message:
                      "Invalid comments format (must be a JSON array of comment objects)",
                    row: rowCount,
                    field: "comments",
                  });
  ...
  async processCustomerCSV(
    file: File,
  ): Promise<CSVParseResult<CustomerProfile>> {
