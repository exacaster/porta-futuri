...
export class ConversationManager {
  ...
  private detectIntent(message: string): 'shopping' | 'general' | 'mixed' {
    const shoppingKeywords = ['buy', 'purchase', 'need', 'looking for', 'price', 'cost'];
    const matchCount = shoppingKeywords.filter(kw => message.includes(kw)).length;
    
    if (matchCount >= 2) {return 'shopping';}
    if (matchCount === 1) {return 'mixed';}
    return 'general';
  }

  private analyzeSentiment(message: string): 'positive' | 'neutral' | 'negative' {
    const positive = ['love', 'great', 'excellent', 'good', 'nice'].some(w => message.includes(w));
    const negative = ['hate', 'bad', 'terrible', 'awful', 'poor'].some(w => message.includes(w));
  ...
  private calculateConfidence(message: string): number {
    ...
    if (message.length > 50) {return 0.8;}
    return 0.6;
  }
  ...
  shouldRedirect(): boolean {
    return this.context.generalTurns >= this.MAX_GENERAL_TURNS &&
