...
export const sanitizeContextEvent = (raw: any): ContextEvent => {
  ...
  if (raw.product_id) {
    event.product_id = String(raw.product_id);
  }
  if (raw.category_viewed) {
    event.category_viewed = String(raw.category_viewed);
  }
  if (raw.search_query) {
    event.search_query = String(raw.search_query);
  }
  if (raw.cart_action) {
    event.cart_action = raw.cart_action as CartAction;
  }
  if (raw.wishlist_action) {
    event.wishlist_action = raw.wishlist_action as WishlistAction;
  }
  if (raw.page_url) {
    event.page_url = String(raw.page_url);
  }
  if (raw.referrer) {
    event.referrer = String(raw.referrer);
  }
  if (raw.device_type) {
    event.device_type = String(raw.device_type);
  }
  if (raw.quantity !== undefined) {
    event.quantity = Number(raw.quantity);
  }
  if (raw.price !== undefined) {
    event.price = Number(raw.price);
  }
  if (raw.filters_applied) {
    event.filters_applied = raw.filters_applied;
  }
