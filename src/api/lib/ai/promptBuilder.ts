...
Remember: You're helping customers find products they'll love, not just selling.`;
  ...
  buildUserPrompt(
    ...
    if (query) {
      ...
      parts.push('TASK: Provide personalized recommendations based on the customer\'s profile and browsing behavior.');
      parts.push('');
    }

    parts.push('Please provide 3-5 product recommendations in JSON format with reasoning and match scores.');
  ...
  private formatCustomerProfile(profile: CustomerProfile): string {
    ...
    if (profile.segment) {lines.push(`- Segment: ${profile.segment}`);}
    if (profile.preferences && profile.preferences.length > 0) {
      lines.push(`- Preferences: ${profile.preferences.join(', ')}`);
    }
    if (profile.purchase_history && profile.purchase_history.length > 0) {
      lines.push(`- Recent Purchases: ${profile.purchase_history.slice(0, 5).join(', ')}`);
    }
  ...
  private formatContext(context: ContextSummary): string {
    ...
    lines.push(`- Browsing Pattern: ${context.intent_signals.browsing_pattern}`);
    if (context.intent_signals.price_sensitivity) {
      lines.push('- Price Sensitive: Yes');
    }
    
    // Real-time context
    if (context.real_time.cart_items && context.real_time.cart_items.length > 0) {
      lines.push(`- Cart Items: ${context.real_time.cart_items.join(', ')}`);
    }
    if (context.real_time.wishlist_items && context.real_time.wishlist_items.length > 0) {
      lines.push(`- Wishlist: ${context.real_time.wishlist_items.join(', ')}`);
    }
    if (context.real_time.browsing_category) {
      lines.push(`- Currently Browsing: ${context.real_time.browsing_category}`);
    }
    if (context.real_time.previous_searches && context.real_time.previous_searches.length > 0) {
      lines.push(`- Recent Searches: ${context.real_time.previous_searches.join(', ')}`);
    }
    
    // Behavior summary
    lines.push(`- Products Viewed: ${context.behavior.unique_products_viewed}`);
    lines.push(`- Categories Browsed: ${context.behavior.categories_browsed.join(', ')}`);
    if (context.behavior.cart_additions > 0) {
      lines.push(`- Cart Activity: ${context.behavior.cart_additions} additions, ${context.behavior.cart_removals} removals`);
    }
  ...
  private formatProductCatalog(products: Product[]): string {
    ...
    Object.entries(byCategory).forEach(([category, categoryProducts]) => {
      ...
      topProducts.forEach(product => {
        ...
        lines.push(`    • ${product.name} (${product.product_id}) - ${price} - ${rating} - Stock: ${stock}`);
      });
    });
  ...
  parseRecommendationResponse(response: string): {
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        return { recommendations, message: message || 'Here are my recommendations for you:' };
      }
      
      // Fallback: try to parse entire response as JSON
      const parsed = JSON.parse(response);
      if (Array.isArray(parsed)) {
        return { recommendations: parsed, message: 'Here are my recommendations for you:' };
      }
    ...
    return {
      recommendations: [],
      message: 'I apologize, but I encountered an issue generating recommendations. Please try again.'
    };
  }
   ...
   */
  buildClarificationPrompt(previousQuery: string, clarificationRequest: string): string {
...
Please provide updated recommendations based on this additional information.`;
  ...
  buildComparisonPrompt(productIds: string[], products: Product[]): string {
    ...
    const productDetails = productsToCompare.map(p => 
