...
export class InsightExtractor {
  ...
  private readonly URGENCY_INDICATORS = {
    immediate: ['asap', 'urgent', 'immediately', 'today', 'right now', 'emergency'],
    researching: ['considering', 'thinking about', 'researching', 'comparing', 'exploring'],
    browsing: ['just looking', 'browsing', 'curious', 'wondering', 'interested']
  };
  ...
  private extractFromMessage(content: string, data: ExtractedData): void {
    ...
    const brandPattern = /\b(Apple|Samsung|Nike|Adidas|Sony|LG|Dell|HP|Lenovo|Microsoft|Amazon|Google)\b/gi;
    const brands = content.match(brandPattern);
    if (brands) {
      brands.forEach(brand => {
        this.addInsight('preference', `brand_${brand.toLowerCase()}`, 0.8, 'explicit');
      });
    }
  }
  ...
  private extractPriceRange(messages: Message[]): [number, number] | null {
    ...
    messages.forEach(msg => {
      if (msg.role === 'user') {
        ...
        const rangeMatch = msg.content.match(/\$?(\d+(?:\.\d{2})?) ?(?:-|to) ?\$?(\d+(?:\.\d{2})?)/);
        if (rangeMatch) {
        ...
        const maxMatch = msg.content.match(/(?:under|below|less than|max|maximum) \$?(\d+(?:\.\d{2})?)/i);
        if (maxMatch) {
        ...
        const minMatch = msg.content.match(/(?:over|above|more than|min|minimum) \$?(\d+(?:\.\d{2})?)/i);
        if (minMatch) {
    ...
    if (hasPriceInfo) {
      return [minPrice === Infinity ? 0 : minPrice, maxPrice === 0 ? 10000 : maxPrice];
    }
    
    return null;
  }
  
  private determineUrgency(messages: Message[]): 'immediate' | 'researching' | 'browsing' | null {
    const combinedText = messages
      .filter(m => m.role === 'user')
      .map(m => m.content.toLowerCase())
      .join(' ');
    
    for (const [urgency, indicators] of Object.entries(this.URGENCY_INDICATORS)) {
  ...
  private analyzeSentiment(messages: Message[]): 'positive' | 'neutral' | 'negative' {
    const positiveWords = ['love', 'great', 'excellent', 'amazing', 'wonderful', 'perfect', 'fantastic', 'awesome'];
    const negativeWords = ['hate', 'bad', 'terrible', 'awful', 'horrible', 'worst', 'disappointing', 'frustrated'];
    ...
    messages.forEach(msg => {
      if (msg.role === 'user') {
        ...
        positiveCount += positiveWords.filter(word => lowerContent.includes(word)).length;
        negativeCount += negativeWords.filter(word => lowerContent.includes(word)).length;
      }
    });
  ...
  getInsightsSummary(): {
    ...
    const urgencyInsight = this.insights.find(i => i.value.startsWith('urgency_'));
    const sentimentInsight = this.insights.find(i => i.value.startsWith('sentiment_'));
    
    // Extract price range from concerns
    const priceRangeInsight = this.insights.find(i => i.value === 'price_range');
    ...
    return {
      ...
      interests,
      priceRange,
      urgency: urgencyInsight ? urgencyInsight.value.replace('urgency_', '') : undefined,
      sentiment: sentimentInsight ? sentimentInsight.value.replace('sentiment_', '') : 'neutral'
    };
  }
