...
export class ConversationManager {
  ...
  private calculateShoppingIntentScore(message: string): number {
    ...
    if (
      /where can i (buy|get|find)/i.test(message) ||
  ...
  private extractEntities(message: string): string[] {
    ...
    const brandPatterns =
      /\b(Apple|Samsung|Nike|Adidas|Sony|LG|Dell|HP|Lenovo|Microsoft)\b/gi;
    ...
    // Extract colors
    const colorPattern =
      /\b(red|blue|green|black|white|gray|grey|yellow|orange|purple|pink)\b/gi;
  ...
  shouldRedirect(): boolean {
    ...
    if (timeSinceShoppingTopic > 60000) {
      // More than 1 minute
      return true;
    }
  ...
  generateRedirectPrompt(topic?: string): string {
    ...
    let template =
      redirectTemplates[Math.floor(Math.random() * redirectTemplates.length)];
    ...
    if (
      this.context.shoppingIntent.category &&
      template.includes("{category}")
    ) {
  ...
  extractInsights(message: string): CustomerInsight[] {
    ...
    const preferencePatterns = [
      {
        pattern: /i (love|like|prefer|enjoy) (\w+)/gi,
        type: "preference" as const,
      },
      {
        pattern: /(\w+) is (my favorite|the best)/gi,
        type: "preference" as const,
      },
      {
        pattern: /i'm (into|interested in) (\w+)/gi,
  ...
  getTransitionSuggestion(): ConversationTransition | null {
    ...
    return (
      transitions.find((t) => t.fromState === this.context.currentState) || null
    );
  }
